...
static HAL_StatusTypeDef I2C_Reset(I2C_HandleTypeDef *hi2c) {
...
HAL_StatusTypeDef AHT20_Init(I2C_HandleTypeDef *hi2c) {
    ...
    status = HAL_I2C_Master_Transmit(hi2c, 0x38 << 1, initCmd, sizeof(initCmd), HAL_MAX_DELAY);
    return status;
}
...
HAL_StatusTypeDef AHT20_ReadTemperatureHumidity(I2C_HandleTypeDef *hi2c, float *temperature, float *humidity) {
    ...
    // Trigger measurement
    status = HAL_I2C_Master_Transmit(hi2c, 0x38 << 1, triggerCmd, sizeof(triggerCmd), HAL_MAX_DELAY);
    if (status != HAL_OK) {
        return status;
    }
    ...
    status = HAL_I2C_Master_Receive(hi2c, 0x38 << 1, data, sizeof(data), HAL_MAX_DELAY);
    if (status != HAL_OK) {
        return status;
    }
